...
class Settings:
                ...
                for key in self.default_settings.keys():
                    if key not in current.keys():
                        ...
                        print("Adding " + str(key) +
                              " field to InsurgentBot settings.json")
    ...
    def parse_cmd_arguments(self):
        parser = argparse.ArgumentParser(description="InsurgentBot - Discord Bot")
        parser.add_argument("--owner", help="ID of the owner. Only who hosts "
                                            "this bot should be owner, this has "
        ...
        parser.add_argument("--admin-role", help="Role seen as admin role by "
                                                 "InsurgentBot")
        parser.add_argument("--mod-role", help="Role seen as mod role by InsurgentBot")
        ...
        parser.add_argument("--no-cogs",
                            action="store_true",
                            help="Starts InsurgentBot with no cogs loaded, only core")
        parser.add_argument("--self-bot",
                            action='store_true',
                            help="Specifies if InsurgentBot should log in as selfbot")
        ...
        parser.add_argument("--dry-run",
                            action="store_true",
                            help="Makes InsurgentBot quit with code 0 just before the "
    ...
    @property
    def token(self):
        return os.environ.get("InsurgentBot_TOKEN", self.bot_settings["TOKEN"])
    ...
    @property
    def email(self):
        return os.environ.get("InsurgentBot_EMAIL", self.bot_settings["EMAIL"])
    ...
    @property
    def password(self):
        return os.environ.get("InsurgentBot_PASSWORD", self.bot_settings["PASSWORD"])
    ...
    @property
    def login_cInsurgentBotentials(self):
        if self.token:
            return (self.token,)
